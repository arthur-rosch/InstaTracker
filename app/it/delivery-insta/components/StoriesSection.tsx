...
// Funzione per normalizzare le Stories
function mapStories(stories: Story[], followers: Follower[]) {
  return stories
    .filter(story => story.image)
    .map((story, index) => {
      // Utilizza l'API proxy per le immagini delle Story
...
// Funzione per normalizzare gli Highlights
function mapHighlights(highlights: Highlight[], followers: Follower[]) {
  ...
  highlights.forEach((highlight) => {
    highlight.items.forEach((item, itemIndex) => {
      if (item.image?.url) {
        // Utilizza l'API proxy per le immagini degli Highlight
...
export default function StoriesSection({
  ...
  // Priorità alle Stories, poi agli Highlights, poi ai dati fittizi
      ...
      <div className="relative z-10">
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h3 className="text-white font-bold text-lg md:text-xl">Storie</h3>
              <p className="text-gray-300 text-sm">Storie che sono state messe in pausa</p>
            </div>
          </div>
                      ...
                      <div className="absolute bottom-3 left-3 right-3">
                        <div className="space-y-2">
                          ...
                          <div className="text-white font-semibold text-sm">
                            {story.pausedProfiles} Profili messi in pausa per +13s
                          </div>
                          {/* Avatars */}
