...
export function HeroSection() {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (stored) {
        ...
        if (data.profile && data.profile.name === cleanUsername) {
          ...
          router.push(`/fr/release-used?username=${cleanUsername}`)
          setIsLoading(false)
          return
        }
      }
    ...
    router.push(`/fr/confirm?username=${cleanUsername}`)
    setIsLoading(false)
  }
        ...
        <div className="container mx-auto px-4 relative z-10">
          <div className="max-w-2xl mx-auto text-center space-y-8">
            ...
            <div className="flex items-center justify-center space-x-3 mb-6">
              ...
              <span className="text-lg font-semibold text-purple-400">Découvre en 1 minute</span>
            </div>

            {/* Main Heading */}
            <div className="space-y-4">
              <h1 className="text-2xl md:text-3xl font-bold text-white leading-tight">
                Toutes les informations que <br />
                <span className="instagram-gradient-text">Instagram te cache.</span>
              </h1>
            </div>
            ...
            <div className="space-y-4">
              <div className="flex flex-col sm:flex-row gap-3 max-w-lg mx-auto">
                <div className="relative flex-1">
                  ...
                  <Input
                    type="text"
                    placeholder="Saisir le nom d'utilisateur Instagram"
                    value={username}
              ...
              <div className="space-y-1">
                <p className="text-xs text-gray-400">100% Anonyme • Résultats instantanés • Aucune inscription requise</p>
                <p className="text-xs text-orange-400 font-semibold">⚠️ Une requête par appareil • Données stockées sur l'appareil</p>
              </div>
            </div>
            ...
            <div className="flex flex-col gap-3 max-w-lg mx-auto mb-6">
              <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-xl p-4 text-left shadow-2xl hover:bg-white/10 transition-all duration-300">
                ...
                <div className="relative z-10">
                  <h3 className="text-sm font-bold text-white mb-1">Profil complet</h3>
                  <p className="text-xs text-gray-300">Accès à toutes les informations du profil, photos, abonnés et plus</p>
                </div>
              </div>
              <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-xl p-4 text-left shadow-2xl hover:bg-white/10 transition-all duration-300">
                <div className="absolute inset-0 bg-gradient-to-br from-pink-500/10 to-transparent rounded-xl"></div>
                <div className="relative z-10">
                  <h3 className="text-sm font-bold text-white mb-1">Stories et messages</h3>
                  <p className="text-xs text-gray-300">Découvre qui a vu tes stories et accède aux messages privés</p>
                </div>
              </div>
            </div>
