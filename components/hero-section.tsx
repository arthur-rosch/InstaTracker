...
export function HeroSection() {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (stored) {
        ...
        if (data.profile && data.profile.name === cleanUsername) {
          ...
          router.push(`/en/release-used?username=${cleanUsername}`)
          setIsLoading(false)
          return
        }
      }
    ...
    router.push(`/en/confirm?username=${cleanUsername}`)
    setIsLoading(false)
  }
          ...
          <div className="max-w-2xl mx-auto text-center space-y-8">
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-1">
                <p className="text-xs text-gray-400">100% Anonymous • Instant Results • No Login Required</p>
            ...
            <div className="flex flex-col gap-3 max-w-lg mx-auto mb-6">
              <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-xl p-4 text-left shadow-2xl hover:bg-white/10 transition-all duration-300">
                ...
                <div className="relative z-10">
                  <h3 className="text-sm font-bold text-white mb-1">Complete Profile</h3>
                  <p className="text-xs text-gray-300">Access all profile information, photos, followers and more</p>
                </div>
              </div>
              <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-xl p-4 text-left shadow-2xl hover:bg-white/10 transition-all duration-300">
                <div className="absolute inset-0 bg-gradient-to-br from-pink-500/10 to-transparent rounded-xl"></div>
                <div className="relative z-10">
                  <h3 className="text-sm font-bold text-white mb-1">Stories and Messages</h3>
                  <p className="text-xs text-gray-300">Find out who viewed your stories and access private messages</p>
                </div>
              </div>
            </div>
